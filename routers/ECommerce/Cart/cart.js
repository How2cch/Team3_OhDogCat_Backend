...
router.post('/orderpostmore/:productId', authMiddleware.authVerify, async (req, res) => {
  ...
  try {
    // 辨認購物車資料庫裡面有沒有相同的商品
    const [isExist] = await pool.execute(`SELECT * FROM cart WHERE user_id=? AND product_id = ? `, [
      user_id,
      product_id,
    ]);
    // 加入購物車
    if (!isExist[0]) {
      console.log('新增');
      result = await pool.execute(`INSERT INTO cart (user_id, product_id, quantity) VALUE (?,?,1)`, [
        user_id,
        product_id,
      ]);
    } else {
      console.log('加一');
      result = await pool.execute(`UPDATE cart SET quantity=quantity+1 WHERE user_id=? AND product_id =? `, [
        user_id,
        product_id,
      ]);
    }
    const isAdd = await pool.execute(
      'SELECT `cart`.`user_id`,`cart`.`product_id`, `cart`.`quantity`, `product`.`name`, `product`.`price`, `product`.`main_photo`, `product`.`photo_path` FROM `cart` JOIN `product` ON `cart`.`product_id` = `product`.`id` WHERE `cart`.`product_id` = ?',[product_id]
    );
    // res.send(
    //   isExist[0] ? { message: '+1' } : { message: '加入購物車' }
    //   // { message: '已成功加入購物車' }
